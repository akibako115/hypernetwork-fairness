...
| `run.json` | `schema_version`、`run_id`、`kind`、状態、開始・終了時刻、Git commit、seed、結果要約を持つ。`kind` は `fit` または `cohort_build`、状態は `running` / `succeeded` / `failed` とする。 |
...
manifest、preflight の順に作成する。予約済みの `run_dir` と checkpoint 出力先は `config.yaml` にも
反映する。呼び出し側は学習後に `succeed`、例外時に `fail` を呼び、
